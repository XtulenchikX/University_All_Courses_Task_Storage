...
class Users_construct:
  ...
  def __init__(self, name: str, height: float, created: str):
    if Users_construct.__check_name(self, name): self.__name = name
    if Users_construct.__check_height(self, height): self.__height = height
    if Users_construct.__check_created(self, created): self.__created = created
